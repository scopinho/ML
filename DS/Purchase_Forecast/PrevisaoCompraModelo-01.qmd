...
# 
#mod_log_01 <- glm(formula = comprou_15_dias ~ ., family = "binomial", data = df_cliente_treino)
...
```{r}
df2 <- select (df_cliente_teste, -comprou_15_dias) 
...
df_final <- cbind (df_cliente_teste, df_fit_class = df_fit)
...
```{r}
caret::confusionMatrix(df_final$df_fit_class, df_final$comprou_15_dias, positive="sim")
```
