...
df_primeira_compra <- ds |> 
  ...
  collect() |> 
  mutate (cliente = as_factor(cliente))
...
df_ultima_compra <- ds |> group_by(cliente) |> summarise(ultima_compra = max(data)) |> 
  collect() |> 
  mutate (cliente = as_factor(cliente)) 
...
df <- df |> arrange(data) |> mutate (recencia = day(days(as_date(ultima_compra) - data)))
...
df_freq <- ds |> group_by(cliente) |> summarise(freq = n()) |> 
  collect() |> 
  mutate (cliente = as_factor(cliente))
