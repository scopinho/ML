...
df_compras <- ds |> select(cliente, data) |> 
  group_by(cliente, data) |> 
  summarise(n=n()) |> 
  arrange (cliente, data) |>  collect()

df_compras<- lazy_dt(df_compras |> select (cliente, data, n))

df_compras <- df_compras |> mutate (diff_1 = data - lag(data),
                                    diff_2 = data - lag(data, n=2),
                                    diff_3 = data - lag(data, n=3))

df_compras <- as_tibble(group_by(df_compras,cliente))


df_compras <- df_compras |> mutate (diff_1 = diff_1 / ddays(1),
                                    diff_2 = diff_2 / ddays(1),
                                    diff_3 = diff_3 / ddays(1))
...
df_compras |>
  write_dataset(path = pq_path3, format = "parquet")
...
df_cliente <- df_cliente |> 
  left_join(select(df_compras, cliente, std, media) |> distinct(), by = "cliente")
...
#Compras (Qtd Produtos e Valor Ultima Compra)
...
df_cliente <- df_cliente |> 
  left_join(ds |> 
              filter (ultima_compra == data) |> 
              select(cliente, descricao_tipo) |>
              group_by(cliente) |> 
              arrange(cliente) |> 
              collect() |> 
              summarise(ultima_compra_qtd_prod = n())  |>
              ungroup() |> 
              collect(), 
            by = "cliente")
