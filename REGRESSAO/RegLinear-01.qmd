...
Veja que este NÃO é uma premissa para usa da técnica, isso é geralmente na validação dos resíduos. Estamos apenas apresentando como fazer o teste e analisando a variável dependente.
...
```{r}
grafico <- scatterplot3d::scatterplot3d(df2$mpg_bc ~ df2$hp + df$wt,
                                        angle = 35, color = "steelblue",box=F)
grafico$plane3d(modelo_multi_2_bc, col="black", draw_polygon = T)
```
